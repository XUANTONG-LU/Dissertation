from eyeSurgeryDatasetModify import EyeSurgeryDataset
...
def main(argc, writer):
    ...
    img_width = 480
    img_height = 288
    trian_ds = EyeSurgeryDataset(root=argc.ds_dir, subset="training", transforms=TrainTransforms(img_width=img_width, img_height=img_height), removeVacantImage=True)
    ...
    test_ds = EyeSurgeryDataset(root="retinalSynthesisDataset", subset="testing", transforms=EvalTransforms(img_width=img_width, img_height=img_height), removeVacantImage=False)
    ...
    with open(log_file_dir, "a") as f:
        ...
        f.write(f"Project: Resnet UNet on modified cataract Dataset")
