...
class EyeSurgeryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.subset == "training":
            # make a binary circle:
            circle = np.zeros_like(binary_mask_np, dtype=np.uint8)
            circle_r = int(binary_mask_np.shape[0]/2)
            centre_coordinates = (int(binary_mask_np.shape[1]/2), int(binary_mask_np.shape[0]/2))
            cv.circle(circle, center=centre_coordinates, radius=circle_r, color=(1, 1, 1), thickness=-1)
            circle_3d = np.dstack([circle, circle, circle])

            # for image:
            img_np = np.array(img, dtype=np.uint8)
            img_seg = circle_3d * img_np
            img = Image.fromarray(np.uint8(img_seg))

            # for mask:
            binary_mask_np = binary_mask_np * circle
        ######
        # convert back to PIL:
...
if __name__ == "__main__":
    ...
    train_ds = EyeSurgeryDataset(root = "retinalSynthesisDataset", subset='training', transforms=TrainTransforms(img_width=480, img_height=288), removeVacantImage=True)
